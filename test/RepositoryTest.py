...
class RepositoryTest(unittest.TestCase):
    ...
    def test_01_reseller_add(self):
        reseller = Reseller(cpf="CPF", email="email@aa.com", password="password")
        duplicate_reseller = Reseller(cpf="CPF", email="email@aa.com", password="password")
    ...
    def test_03_reseller_login(self):
        ...
        with self.assertRaises(NotFoundException):
            self.reseller_repository.login(cpf="CPF", password="passwords")
        assert data.cpf == "CPF"

    def test_04_purchase_add(self):
        reseller = self.reseller_repository.find_by_cpf(cpf="CPF")
        print(reseller)
        purchase = Purchase(cpf="CPF", date=datetime.now()
